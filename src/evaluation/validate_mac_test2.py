...
class OneShotApp(Application):
    ...
    def send_packet(self):
        ...
        print(
            f"[{self.host.context.scheduler.now():.6f}s] [{self.host.id}] [App] > Attempting to send '{self.payload.decode('utf-8','ignore').split('#')[0]}'."
        )
...
def run_mac_test2():
    ...
    app_A = OneShotApp(
        node_A,
        dest_addr=addr_node_B,
        time_to_send=1.0,
        payload_name="LONG_PACKET",
        size_bytes=100,
    )
    app_C = OneShotApp(
        node_C,
        dest_addr=addr_node_B,
        time_to_send=1.001,
        payload_name="SHORT_PACKET",
        size_bytes=20,
    )
    app_B = OneShotApp(
        node_B,
        dest_addr=addr_node_A,
        time_to_send=float("inf"),
        payload_name="",
        size_bytes=0,
    )
