...
def calculate_bounds_and_params(node_positions, padding=50, dspace_step=1.0) -> int:
    ...
    print(f"Topology bounds: X=[{min_x:.1f}, {max_x:.1f}], Y=[{min_y:.1f}, {max_y:.1f}]")
    print(f"DSpace params: step={dspace_step}, npt={dspace_npt} (Grid span approx. [{-half_n*dspace_step:.1f}, {half_n*dspace_step-dspace_step:.1f}])")
    return dspace_npt
...
def main():
    ...
    parser.add_argument("--app_delay", type=float, default=60.0, help="Delay before applications start")
    parser.add_argument("--mean_interarrival", type=float, default=60.0, help="Mean inter-arrival time for PoissonTrafficApp")
    parser.add_argument("--dspace_step", type=float, default=1.0, help="Resolution of the DSpace grid (meters)")
    ...
    node_addrs_by_index = {i: (i + 1).to_bytes(2, "big") for i in range(actual_num_nodes)}
    ...
    for i in range(actual_num_nodes):
        ...
        if args.app == "pingpong":
            ...
            if pinger_idx is None: pinger_idx = 1 if actual_num_nodes > 1 else 0
            if ponger_idx is None: ponger_idx = actual_num_nodes - 1
            if pinger_idx == 0: pinger_idx = 1 # Pinger cannot be sink
            
            is_pinger = (i == pinger_idx)
            peer_addr = node_addrs_by_index.get(ponger_idx) if is_pinger else None
            app_instance = PingPongApp(host=None, is_pinger=is_pinger, peer_addr=peer_addr, start_delay=args.app_delay)
            
            if is_pinger: role = "pinger"
            if i == ponger_idx: role = "ponger"
