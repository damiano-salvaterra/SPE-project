...
class OneShotApp(Application):
    ...
    def send_packet(self):
        ...
        print(
            f"[{self.host.context.scheduler.now():.6f}s] [{self.host.id}] [App] > Attempting to send '{self.payload_str}'."
        )
...
def run_mac_test3():
    ...
    app_A = OneShotApp(
        node_A, dest_addr=addr_node_B, time_to_send=1.0, payload="Packet from A"
    )
    app_C = OneShotApp(
        node_C, dest_addr=addr_node_B, time_to_send=1.0, payload="Packet from C"
    )
    app_B = OneShotApp(
        node_B, dest_addr=addr_node_A, time_to_send=float("inf"), payload=""
    )
