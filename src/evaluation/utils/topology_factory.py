...
class TopologyStrategy(ABC):
    """
    Abstract base class (Strategy) for a topology generator.
    Each strategy must implement the generate_positions method.
    """
    ...
    def generate_positions(self, **kwargs) -> List[CartesianCoordinate]:
        ...
        Args:
            **kwargs: Specific parameters for the topology 
                      (e.g., num_nodes, node_distance, radius).
                      
        Returns:
            List[CartesianCoordinate]: A list of node positions.
        """
        pass
...
class RingTopology(TopologyStrategy):
    """Generates nodes evenly distributed on a circle."""
...
class RandomTopology(TopologyStrategy):
    ...
    def generate_positions(self, **kwargs) -> List[CartesianCoordinate]:
        ...
        # area_box = (min_x, max_x, min_y, max_y)
        area_box: Tuple[float, float, float, float] = kwargs.get("area_box", (-100, 100, -100, 100))
        # Requires a numpy random generator to be passed
        rng: np.random.Generator = kwargs.get("rng")
        
        if rng is None:
            raise ValueError("RandomTopology requires an 'rng' (NumPy Random Generator) in kwargs")
...
class StarTopology(TopologyStrategy):
    ...
    def generate_positions(self, **kwargs) -> List[CartesianCoordinate]:
        num_nodes: int = kwargs.get("num_nodes", 11) # 1 hub + 10 spokes
        radius: float = kwargs.get("radius", 100.0)
        center_x: float = kwargs.get("center_x", 0.0)
        center_y: float = kwargs.get("center_y", 0.0)
...
class ClusterTreeTopology(TopologyStrategy):
    """
    Generates a simple cluster-tree: a root, N clusters, M nodes per cluster.
    Note: This is a simple spatial arrangement, not the logical topology.
    """
    
    def generate_positions(self, **kwargs) -> List[CartesianCoordinate]:
        num_clusters: int = kwargs.get("num_clusters", 3)
        nodes_per_cluster: int = kwargs.get("nodes_per_cluster", 5)
        cluster_radius: float = kwargs.get("cluster_radius", 100.0) # Distance of clusters from root
        node_radius: float = kwargs.get("node_radius", 20.0) # Radius of nodes within a cluster
        center_x: float = kwargs.get("center_x", 0.0)
        center_y: float = kwargs.get("center_y", 0.0)
        # Requires a numpy random generator
        rng: np.random.Generator = kwargs.get("rng")

        if rng is None:
            raise ValueError("ClusterTreeTopology requires an 'rng' (NumPy Random Generator) in kwargs")
        ...
        for i in range(num_clusters):
            ...
            for _ in range(nodes_per_cluster - 1): # -1 because CH is a node
                node_angle = rng.uniform(0, 2 * math.pi)
                node_dist = rng.uniform(0, node_radius)
...
class TopologyFactory:
    """
    The Factory class that builds a topology.
    It holds a registry of all available topology strategies.
    """
    ...
    def _register_default_strategies(self):
        """Registers the built-in topology types."""
    ...
    def register_strategy(self, name: str, strategy: TopologyStrategy):
        """Allows adding new topology strategies at runtime."""
        self._strategies[name.lower()] = strategy

    def create_topology(self, name: str, **kwargs) -> List[CartesianCoordinate]:
        """
        Creates and generates a topology by its registered name.
        
        Args:
            name (str): The name of the topology (e.g., "linear", "ring").
            **kwargs: Parameters to pass to the strategy's 
                      generate_positions method.
                      
        Returns:
            List[CartesianCoordinate]: The list of node positions.
        ...
        if not strategy:
            raise ValueError(f"Topology strategy '{name}' is not registered. "
