...
class OneShotApp(Application):
    ...
    def start(self):
        if self.time_to_send < float('inf'):
            ...
            print(f"[{self.host.context.scheduler.now():.6f}s] [{self.host.id}] [App] Scheduled packet send for t={self.time_to_send:.2f}s.")

    def send_packet(self):
        payload_str = f"Test1 from {self.host.id}"
        packet = NetPacket(APDU=payload_str.encode('utf-8'))
        print(f"[{self.host.context.scheduler.now():.6f}s] [{self.host.id}] [App] > Sending packet to {self.dest_addr.hex()}.")
