...
from simulator.applications.PingPongApplication import PingPongApp
...
def main(num_nodes: int = 2, node_distance: int = 30, simulation_time: float = 600.0, root_seed: int = 12345, bootstrap_params: dict = None, bootstrapped_kernel: Kernel = None):
    """
    ...
    print(f"Network configuration: num_nodes={num_nodes}, node_distance={node_distance}")
    ...
    if bootstrapped_kernel:
        ...
        if bootstrap_params:
            ...
            params_str = ", ".join([f"{k}={v}" for k, v in bootstrap_params.items() if k != "seed"])
        ...
        print("No bootstrapped kernel provided. Creating one with default stable channel parameters.")
        ...
        params_str = ", ".join([f"{k}={v}" for k, v in default_stable_params.items() if k != "seed"])
        print(f"Kernel configuration: {params_str}")
    ...
    if queue_len > 0:
        ...
        for i, (time, event) in enumerate(sorted(scheduler.event_queue)[-10:]):
            print(f"{queue_len - 10 + i + 1}: t={time * scheduler._time_scale:.6f}s, Event={type(event).__name__}, Blame={type(event.blame).__name__}, Descriptor: {event.descriptor}, Cancelled: {event._cancelled}")
