...
class InterarrivalTimeMonitor(Monitor):
    ...
    def __init__(self, monitor_name: str = "IT", verbose=True):
        ...
        self.arrival_times_map = (
            {}
        )  # key: emitting host linkaddr, value: list of arrival times
    ...
    def update(self, entity: "Entity", signal: "EntitySignal"):
        ...
        if not isinstance(
            signal, (AppSendSignal, AppProcessStartSignal, AppSendFailSignal)
        ):
        ...
        elif isinstance(signal, (AppSendFailSignal, AppSendSignal)):

            if host_addr not in self.arrival_times_map.keys():
                self.arrival_times_map[host_addr] = (
                    []
                )  # create the key if it missed the process start, there is no reference anyways since hte diffs are computed at the end
    ...
    def save_to_csv(self, base_path: str):
        ...
        log_row = (
            {}
        )  # row of the csv: one column for each host. Only one row with the average IT
