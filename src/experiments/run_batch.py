...
TOPOLOGIES = ["linear", "ring", "grid", "random", "star"]
CHANNELS = ["ideal", "stable", "stable_mid_pl", "stable_high_pl", "lossy", "unstable"]
NUM_NODES = 20
TX_POWER = 0.0  # in dBm
...
PYTHON_EXE = (
    sys.executable
)  # Use the same Python interpreter running this script (for venv activation)
...
def run_job_worker(job_params: dict) -> Tuple[dict, bool, str]:
    ...
    command = [
        ...
        job_params["channel"],
        "--num_nodes",
        str(job_params["num_nodes"]),
        "--tx_power",
        str(job_params["tx_power"]),
        "--sim_time",
        str(job_params["sim_time"]),
        "--sim_seed",
        str(job_params["sim_seed"]),
        "--topo_seed",
        str(job_params["topo_seed"]),
        "--app_delay",
        str(job_params["app_delay"]),
        "--mean_interarrival",
        str(job_params["mean_interarrival"]),
        "--dspace_step",
        str(job_params["dspace_step"]),
        "--out_dir",
        job_params["out_dir"],
    ]
...
def main_orchestrator():
    ...
    if args.antithetic:
        if args.replications % 2 != 0:
            print(
                f"ERROR: Antithetic mode requires an even number of replications (got {args.replications}).",
                file=sys.stderr,
            )
            print(
                "Please use -n 100 (for 50 pairs), -n 20 (for 10 pairs), etc.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        print(
            f"Antithetic mode enabled. Generating {num_pairs} pairs ({args.replications} total runs)..."
        )
        ...
        print(
            f"Standard mode. Generating {args.replications} independent replications..."
        )
        for topo, chan in param_sweep:
    ...
    if num_workers < args.workers:
        print(
            f"Warning: Number of workers ({args.workers}) exceeds number of jobs ({num_jobs}). Using {num_workers} workers."
        )
    ...
    with multiprocessing.Pool(processes=num_workers) as pool:
        try:
            ...
            for job_params, success, message in tqdm(
                results_iterable, total=num_jobs, desc="Running Jobs"
            ):
                if not success:
            ...
            print(
                "\n!!! KEYBOARD INTERRUPT RECEIVED !!! Forcibly terminating workers...",
                file=sys.stderr,
            )
            pool.terminate()
            pool.join()
