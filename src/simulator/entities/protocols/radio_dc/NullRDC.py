...
class NullRDC(Layer, Entity):
    ...
    def uc_tx_outcome(
        self, rx_addr: bytes, status_ok: bool, num_tx: int, ack_rssi: float
    ):
        self.host.net._uc_sent(
            rx_addr=rx_addr, status_ok=status_ok, num_tx=num_tx, ack_rssi=ack_rssi
        )
    ...
    def _sense_channel(self, frame: Frame_802_15_4):
        ...
        busy = (
            self.host.phy.cca_802154_Mode1()
        )  # TODO: check if broadcast packets perform CCA or not, in case skip it fro broadcasts
        if not busy:  # if channel is free, transmit
            self.host.phy.send(
                payload=frame
            )  # TODO: i think there is a small period between the backoff and the actual send, check if we should model it
        else:
