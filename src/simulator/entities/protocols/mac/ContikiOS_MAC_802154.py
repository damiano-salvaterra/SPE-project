...
class ContikiOS_MAC_802154_Unslotted(Layer, Entity):
    ...
    def on_RDCSent(self, packet: MACFrame):
        ...
        if self.current_output_frame._requires_ack:
            self.state = MACState.AWAITING_ACK # if you sent a packet that requires ack, wait for it setting the relative state
        ...
        if isinstance(payload, Frame_802154):
            ...
            if self.state == MACState.AWAITING_ACK and self.current_output_frame and payload.seqn == self.current_output_frame.seqn:
                if self.pending_ack_timeout_event:
                    self.host.context.scheduler.unschedule(self.pending_ack_timeout_event) #unschedule the ack timeout
