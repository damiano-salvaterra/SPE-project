...
class RandomManager:
    def __init__(self, root_seed: int = 0, worker_id = 0) -> None:
        ...
        self._worker_seed = [worker_id, root_seed] # to ensure unique seeds for each worker
        self._engines: Dict[str, Generator] = {}
    ...
    def create_stream(self, key: str) -> None:
        ...
        key_hash = int.from_bytes(hashlib.sha256(key.encode()).digest(), 'little') # get a stable hash for the key
        stream_seed = [key_hash] + self._worker_seed # create a unique seed for the stream
        bitgen = Philox(stream_seed) # init the new bitgen 
        rng = Generator(bitgen) # create new variate generator
        self._engines[key] = rng # add the new stream to the dictionary


    def get_stream(self, key: str) -> Generator:
        key = key.lower()
