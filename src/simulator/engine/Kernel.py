...
class Kernel:
    ...
    def __init__(self, root_seed=0, antithetic: bool = False) -> None:
        """Init the kernel and its core components."""
        self.context = SimulationContext(
            scheduler=Scheduler(),
            random_manager=RandomManager(root_seed=root_seed, antithetic=antithetic),
        )
    ...
    def bootstrap(
        ...
        self.dspace = DSpace(dspace_step=dspace_step, dspace_npt=dspace_npt)
    ...
    def set_parameters(
        self, **kwargs
    ):  # TODO: generated by copilot, it has to be changed
        """
        ...
        print(
            f"Setting parameters: {kwargs}. Note: a re-bootstrap may be needed for some changes."
        )
    ...
    def remove_node(
        self, node_id: str
    ):  # TODO: check if the node has current sessions or other open states before removing.
    ...
    def run(self, until: float = float("inf")):
        ...
        while (
            not self.context.scheduler.is_empty()
            and self.context.scheduler.now() <= until
        ):
            self.step()
    ...
    def get_entity(self, entity_path: str) -> Optional[Entity]:
        ...
        if not isinstance(entity, Entity):
            print(
                f"Error: Attribute '{parts[1]}' of node '{parts[0]}' is not a valid Entity."
            )
            return None
