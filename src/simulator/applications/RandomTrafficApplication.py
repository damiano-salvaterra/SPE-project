...
from simulator.applications.common.app_events import (
    AppSendEvent,
    AppWaitNetworkBootEvent,
)
...
class RandomTrafficApplication(Application):
    ...
    def __init__(
        ...
        super().__init__()
        self.host = host
        self.destinations: Dict[str, bytes] = {
            id: addr for id, addr in nodes.items if id != self.host.id
        }  # dictionary of destinations string_id : linkaddr
    ...
    def _wait_network_bootstrap(
        self,
    ):  # wait for some time for the network to be built. Then start generating traffic
        wait_time = 10  # wait 10 seconds
        wait_event_time = self.host.context.scheduler.now() + wait_time
        wait_event = AppWaitNetworkBootEvent(
            time=wait_event_time, blame=self, callback=self._sc
        )
    ...
    def _schedule_next_packet(self):
        ...
        send_event = AppSendEvent(
            time=next_send_time, blame=self, callback=self._send_packet
        )
    ...
    def _send_packet(self):
        if not self.destinations:
            ...
            return
        destination_id = self.rng.choice(
        ...
        self.host.net.send(
            payload=payload, destination=self.destinations[destination_id]
        )
