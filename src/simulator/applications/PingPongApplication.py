...
class PingPongApp(Application):
    ...
    '''
    def __init__(self, host: Optional["StaticNode"], is_pinger: bool = False, peer_addr: Optional[bytes] = None, ping_interval: float = 15.0):
        super().__init__()
        self.host = host
    ...
    def start(self):
        ...
        if self.is_pinger:
            ...
            print(f"{self.__class__.__name__}: Scheduling first PING at t={initial_send_time:.2f}s.")
    ...
    def generate_traffic(self):
        ...
        print(f"{self.__class__.__name__}: >>> Attempting to send '{payload_str}' to {self.peer_addr.hex()}.")
        ...
        if not sent_successfully and self.is_pinger:
            ...
            print(f"{self.__class__.__name__}: Send failed. Retrying PING at t={retry_time:.2f}s.")
    ...
    def receive(self, packet: NetPacket, sender_addr: bytes):
        ...
        print(f"{self.__class__.__name__}: <<< Received '{payload_str}' from {sender_addr.hex()}.")
        ...
        if "PING" in payload_str and not self.is_pinger:
            ...
            try:
                ...
                reply_payload_str = f"PONG #{seqn} in response to 'PING #{seqn} from {self.host.id}'"
            except (IndexError, ValueError):
                reply_payload_str = f"PONG in response to '{payload_str}'"
                
            reply_packet = NetPacket(APDU=reply_payload_str.encode('utf-8'))
            print(f"{self.__class__.__name__}: >>> Replying with '{reply_payload_str}' to {sender_addr.hex()}.")
        ...
        if "PONG" in payload_str and self.is_pinger:
            ...
            print(f"{self.__class__.__name__}: PONG received. Scheduling next PING at t={next_ping_time:.2f}s.")
